...
import listeners.global_area.projectInformation.ProjectInformationHandler_old
...
class ProjectInformationConsistencyJob {
    ...
    private final ProjectInformationHandler_old handler = new ProjectInformationHandler_old()
    ...
    void run() {
        ...
        ApplicationUser writeUser = userManager.getUserByName(ProjectInformationHandler_old.JIRA_BOT)

        if (!writeUser) {
            log.error("Project Information consistency job: service user '${ProjectInformationHandler_old.JIRA_BOT}' not found. Aborting.")
            return
        }
        ...
        try {
            ...
            List<Issue> initiatives = handler.findIssuesAsUserPaged(
                    writeUser,
                    "issuetype = \"${ProjectInformationHandler_old.ISSUE_TYPE_INITIATIVE}\""
            )
    ...
    private void validateConfiguration() {
        ...
        if (!cfManager.getCustomFieldObject(ProjectInformationHandler_old.CF_PROJECT_INFORMATION_ID)) {
            missing.add("CF_PROJECT_INFORMATION_ID=${ProjectInformationHandler_old.CF_PROJECT_INFORMATION_ID}")
        }

        if (!cfManager.getCustomFieldObject(ProjectInformationHandler_old.CF_PROJECT_INFORMATION_TEXT_ID)) {
            missing.add("CF_PROJECT_INFORMATION_TEXT_ID=${ProjectInformationHandler_old.CF_PROJECT_INFORMATION_TEXT_ID}")
        }

        if (!cfManager.getCustomFieldObject(ProjectInformationHandler_old.CF_PROJECT_INFORMATION_FLAG_ID)) {
            missing.add("CF_PROJECT_INFORMATION_FLAG_ID=${ProjectInformationHandler_old.CF_PROJECT_INFORMATION_FLAG_ID}")
        }

        if (!cfManager.getCustomFieldObject(ProjectInformationHandler_old.CF_PARENT_LINK)) {
            missing.add("CF_PARENT_LINK=${ProjectInformationHandler_old.CF_PARENT_LINK}")
        }

        if (!ProjectInformationHandler_old.CF_EPIC_LINK || !cfManager.getCustomFieldObject(ProjectInformationHandler_old.CF_EPIC_LINK)) {
            missing.add("CF_EPIC_LINK=${ProjectInformationHandler_old.CF_EPIC_LINK}")
        }
    ...
    private void checkBrowsePermissions(ApplicationUser serviceUser) {
        ...
        projects.each { Project project ->
            if (ProjectInformationHandler_old.EXCLUDED_PROJECT_KEYS.contains(project.key)) {
                return
            }
    ...
    private void validateTree(
        ...
        if (initiative) {
            ...
            expectedPi = currentPi
            expectedFlag = ProjectInformationHandler_old.FLAG_OVERRIDE_ROOT
            childValue = currentPi
            childInheritedFromOverride = true
        } else if (inheritedFromOverride) {
            branch = "INHERITED_FROM_OVERRIDE"
            expectedPi = inheritedValue
            expectedFlag = ProjectInformationHandler_old.FLAG_INHERITED_FROM_OVERRIDE
            childValue = inheritedValue
            childInheritedFromOverride = true
        } else {
            branch = "INHERITED_FROM_INITIATIVE"
            expectedPi = inheritedValue
            expectedFlag = ProjectInformationHandler_old.FLAG_INHERITED_FROM_INITIATIVE
            childValue = inheritedValue
            childInheritedFromOverride = false
        }
    ...
    private boolean isEffectiveOverrideRoot(
        ...
        if (flag == null || flag != ProjectInformationHandler_old.FLAG_OVERRIDE_ROOT_INT) {
            return false
        }
    ...
    private void sendReport(ApplicationUser writeUser) {
        ...
        String permissionSection = projectsWithoutBrowse.isEmpty() ?
                ...
                "Browse permission     : MISSING for '${ProjectInformationHandler_old.JIRA_BOT}' on ${projectsWithoutBrowse.size()} project(s):\n" +
        ...
        Excluded projects     : ${ProjectInformationHandler_old.EXCLUDED_PROJECT_KEYS.isEmpty() ? "none" : ProjectInformationHandler_old.EXCLUDED_PROJECT_KEYS.join(", ")}
        ${permissionSection}
