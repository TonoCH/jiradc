AJS.toInit(function () {
  ...
  function loadLiveOverlay() {
    ...
    fetch(url, { method: 'GET', credentials: 'include' })
      ...
      .then(function (data) {
        if (!data || data.error) {
          if (line) {
            line.innerHTML = '<span style="color:#FF5630;">Live error: '
        ...
        // Update the same "latest week" line in-place (current week, live) —
        // no separate gauge, so the chart doesn't duplicate.
        if (line) {
          line.innerHTML = 'Live now <strong>W' + escapeHtml(data.week || '-') + '</strong>: '
            + formatPercent(pct)
            + ' <span style="color:#6B778C;">(' + toNumber(data.questionsCount) + ' questions, '
            + toNumber(data.elapsedMs) + ' ms)</span>';
        }
      ...
      .catch(function (err) {
        if (line) {
          line.innerHTML = '<span style="color:#FF5630;">Live fetch failed: ' + escapeHtml(String(err)) + '</span>';
        }
      })
      .then(function () {
  ...
  function renderOverallGauge(data) {
    ...
    host.innerHTML = ''
      ...
      + '<div class="kvs-gauge-sub" id="kvs-latest-week-line">Latest week <strong>' + escapeHtml(latestWeek) + '</strong>: '
