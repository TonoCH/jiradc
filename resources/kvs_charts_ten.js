AJS.toInit(function () {
  ...
  function initLevel1Checklist() {
    ...
    function l1RenderTable(data) {
      ...
      // Unify question-row height: measure the tallest row after layout, then
      // force every data row to that height. Keeps check cells visually aligned
      // regardless of whether a question wraps to 1, 2 or 3 lines.
      l1EqualizeRowHeights();
    ...
    function l1EqualizeRowHeights() {
      var rows = tableWrap.querySelectorAll('table.l1-table tbody tr:not(.l1-signature-row)');
      if (!rows.length) return;
      // reset any previous override before measuring
      for (var i = 0; i < rows.length; i++) rows[i].style.height = '';
      var maxH = 0;
      for (var j = 0; j < rows.length; j++) {
        if (rows[j].offsetHeight > maxH) maxH = rows[j].offsetHeight;
      }
      if (maxH <= 0) return;
      for (var k = 0; k < rows.length; k++) rows[k].style.height = maxH + 'px';
    }
    ...
    function l1BuildPrintHeader(data, kw, dateStr, wp, visDays, t) {
      ...
      // Row 1: blank corner + workplace names (header).
      // Row 2: "Verantwortlich:" label + blank cells aligned with names above,
      //        so the label sits on the same line where the names get written.
      if (wp && wp.length) {
        h += '<table class="l1-rh-resp"><tr>';
        h += '<td class="l1-rh-label"></td>';
        for (var wi = 0; wi < wp.length; wi++) {
          h += '<td class="l1-rh-wp-name">' + escapeHtml(wp[wi].name) + '</td>';
        }
        h += '</tr><tr>';
        h += '<td class="l1-rh-label l1-rh-resp-blank">' + escapeHtml(t.responsiblePerson) + ':</td>';
