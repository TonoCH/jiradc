...
Logger log = Logger.getLogger("PF_cloneIssue")
...
CustomField cfrootcausecategory = CustomFieldUtil.getCustomFieldByName(CustomFieldNameConstants.ROOT_CAUSE_CATEGORY)

//def cfManager = ComponentAccessor.getCustomFieldManager()
//CustomField cfrootcausecategory = cfManager.getCustomFieldObject("customfield_16100")
...
boolean isSiblingType = cfCloningType && currentIssue.getCustomFieldValue(cfCloningType) && currentIssue.getCustomFieldValue(cfCloningType).value == "Sibling"
log.info("isSiblingType - ${isSiblingType}" )

def parentIssue = currentIssue

if (isSiblingType) {
    parentIssue = currentIssue.getCustomFieldValue(cfParentBug) as MutableIssue
}
...
newIssue.setLabels(parentIssue.labels)
...
//Root cause category
if(cfrootcausecategory == null){
    log.error("PF_cloneIssue : cfrootcausecategory is null")
    return
}

def rootCauseValue = parentIssue.getCustomFieldValue(cfrootcausecategory)?.value
log.info("Root Cause to clone: ${rootCauseValue}")

if (rootCauseValue) {
    def optionsManager = ComponentAccessor.getOptionsManager()
    def config = cfrootcausecategory.getRelevantConfig(newIssue)
    def allOptions = optionsManager.getOptions(config)
    def optionToSet = allOptions.find { it.value == rootCauseValue }

    if (optionToSet) {
        newIssue.setCustomFieldValue(cfrootcausecategory, optionToSet)
    } else {
        log.warn("Root cause option '${rootCauseValue}' not found in target config")
        parentCommentStr += "\n*NOTE*: Root Cause '${rootCauseValue}' not set – option not available in target project."
    }
}
...
// Create comment
if (isSiblingType) {
    // if sibling created close current issue
    String commentStr = addComment("issue sibling-cloned from ${currentIssue.key}", comment)
    ComponentAccessor.getCommentManager().create(newIssue, currentUser, commentStr, false)

    commentStr = addComment("issue sibling-cloned to ${newIssue.key}", comment)
    ComponentAccessor.getCommentManager().create(currentIssue, currentUser, commentStr, false)
    
    parentCommentStr = addComment("issue sibling-cloned from ${currentIssue.key} to ${newIssue.key}", comment)
} else {
...
currentIssue.setCustomFieldValue(cfCloningType, null)
