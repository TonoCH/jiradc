...
Logger log = Logger.getLogger("PF_psBackToTodoOnParent")
...
if(parentIssue.status.name != "In Progress") {
	return
}

String commentForParent = "Set child issue ${issue.key} back to todo"
...
// if there are other child bugs for the parent bug, which are in progress, no further action is required
...
if (childIssues.findAll{it.status.name == "In Progress"}) {
	log.info("Found child issues, which are in progress: ${childIssues.size()}")
	return
}

// create needed parameter map for automated transition of parent
Map params = new HashMap()
params.put("comment", "set status back to Todo automatically because it has been done on child issue")
TransitionIssueUtil.transitionIssueByTransitionName(parentIssue, "Back to Todo", params)
	
String commentForChild = "set status of parent issue ${parentIssue.key} back to Todo automatically"
