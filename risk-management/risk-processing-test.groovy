...
public void riskMatrix() {
    ...
    if(colorName == null){
        ...
        * */
        ...
        int transitionActionId = colorName == RiskManagement.GREEN ? 321 : 71
        //String currentParentStatus = parentIssue.getStatusObject().getName();    
        
        if (parentIssue.getStatusObject().getName().equals("Created")) {
            processParentWorkflow((MutableIssue) parentIssue, 31, colorName, false);//Assign Risk Owner -> Verification
        }  

        if (parentIssue.getStatusObject().getName().equals("Verification")) {
            processParentWorkflow((MutableIssue) parentIssue, 51, colorName, false);//Examine Risk Actions -> Examine Actions 
        }         

        if (parentIssue.getStatusObject().getName().equals("Examine Actions")) {
            processParentWorkflow((MutableIssue) parentIssue, 61, colorName, false);//Assess Risk -> In Assessment
        } 
        if (parentIssue.getStatusObject().getName().equals("In Assessment")) {
            //IN_ASSESSMENT => HANDLING OR IN_ASSESSMENT => PROCESSED
            processParentWorkflow((MutableIssue) parentIssue, transitionActionId, colorName, true);
        } 
...
public void processParentWorkflow(MutableIssue parent, int actionId, String colorName, boolean doComment){
    log.debug("processParentWorkflow: " + actionId + "; colorName: " + colorName);
...
/*public void processParentWorkflowOld(String colorName, MutableIssue parent){//, String issueKey){
